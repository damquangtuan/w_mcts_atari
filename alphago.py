...
from helpers import (argmax,is_atari_game,copy_atari_state,store_safely,restore_atari_state,stable_normalizer,power,compute_prob_max)
...
class Action():
    ...
    def add_child_state(self,s1,r,terminal,model):
        self.child_state = State(s1,r,terminal,self,self.parent_state.na,model,tau=self.tau,epsilon=self.epsilon,
                                 lambda_const= self.lambda_const,algorithm=self.algorithm,p=self.p,std=0)
...
class State():
    ...
    def __init__(self,index,r,terminal,parent_action,na,model,tau,epsilon,lambda_const,algorithm,p,std=0):
    ...
    def update(self, update_type='mean'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--game', default='Breakout-v0',help='Training environment')
